...
def create_saham_visualizations():
    ...
    with col1:
        ...
        st.info("💡 Vous pouvez uploader **plusieurs fichiers TXT** (ex: si votre fichier dépasse 200MB, découpez-le en parties)")
        f1_list = st.file_uploader(
            "Fichier(s) TXT principal(aux)",
            type=["txt", "csv"],
            key="saham_f1_viz",
        ...
        if f1_list:
            try:
                ...
                for f1 in f1_list:
                    ...
                    df_part = lire_fichier_principal_saham(raw_str)
                    all_dfs.append(df_part)
                df_f1 = pd.concat(all_dfs, ignore_index=True) if len(all_dfs) > 1 else all_dfs[0]
                st.session_state.saham_credits_depots_raw = df_f1
                save_cache("saham_credits_depots_raw", df_f1)
                st.success(f"✅ {len(f1_list)} fichier(s) combinés — {len(df_f1):,} lignes au total")
            ...
            with col1:
                ...
                if f1_tab_list:
                    try:
                        ...
                        st.success(f"✅ {len(f1_tab_list)} fichier(s) combinés — {len(df_f1t):,} lignes au total")
                        with st.expander("Aperçu"):
