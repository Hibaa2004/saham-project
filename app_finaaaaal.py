...
def balance_module():
    ...
    with tab4:
        ...
        for _k in ['immos_col_pcec_f1', 'immos_mode_mont',
                   'immos_col_mont_f1', 'immos_col_cred_f1', 'immos_col_debit_f1',
                   'immos_col_pcec_f2', 'immos_col_pci_f2',
                   'immos_t1_pci_col',  'immos_t1_mont_col', 'immos_res_t1',
                   'immos_t2_pci_col',  'immos_t2_mont_col', 'immos_res_t2',
                   'immos_t3_pci_col',  'immos_t3_mont_col', 'immos_t3_label', 'immos_res_t3']:
            if _k not in st.session_state:
                st.session_state[_k] = None
        ...
        if not all_loaded:
            ...
            # ── Helpers vectorisés (partagés entre tous les tableaux) ─────────
            ...
            def clean_code(series):
                s = (series.astype(str).str.strip().str.strip('"')
                     ...
                     .str.lstrip('0'))  # supprime zeros de tete : '0566750' -> '566750'
                # si la valeur etait '0' ou '000', lstrip donne '' -> remettre '0'
                return s.where(s != '', '0')


            # ── Préparer Balance PCEC (commune aux 3 tableaux) ────────────────
            def get_balance_df():
            ...
            # ── Préparer Mapping PCI→PCEC (commun aux 3 tableaux) ────────────
            def get_mapping_df():
                c_pcec = st.session_state.immos_col_pcec_f2
                c_pci  = st.session_state.immos_col_pci_f2
                if c_pcec is None or c_pci is None:
                    return None
                df_m = df_map[[c_pcec, c_pci]].copy()
                df_m.columns = ['PCEC','PCI']
                df_m['PCEC'] = clean_code(df_m['PCEC'])
                df_m['PCI']  = clean_code(df_m['PCI'])
                return (df_m.dropna(subset=['PCEC','PCI'])
                        .query('PCEC.str.len()>0 and PCI.str.len()>0'))

            def build_table(pci_col, mont_col, label_mont, diag_key=None):
                df_b = get_balance_df()
                df_m = get_mapping_df()
                if df_b is None or df_m is None:
                    return pd.DataFrame()

                # ── Fichier 3 : PCI + montant ─────────────────────────────────
                ...
                # ── Diagnostic : quels PCI matchent le mapping ? ──────────────
                pci_fichier3 = set(df_i['PCI'].unique())
                pci_mapping  = set(df_m['PCI'].unique())
                pci_communs  = pci_fichier3 & pci_mapping
                pci_absents  = pci_fichier3 - pci_mapping

                if diag_key:
                    with st.expander(f"🔍 Diagnostic jointure ({diag_key})", expanded=len(pci_communs)==0):
                        c1, c2, c3 = st.columns(3)
                        c1.metric("PCI dans fichier 3",  len(pci_fichier3))
                        c2.metric("PCI dans mapping",    len(pci_mapping))
                        c3.metric("PCI en commun",       len(pci_communs),
                                  delta_color="normal" if len(pci_communs)>0 else "inverse",
                                  delta="✅ OK" if len(pci_communs)>0 else "❌ 0 match")
                        if len(pci_communs) == 0:
                            st.error("❌ Aucun PCI en commun — les codes ne correspondent pas.")
                            st.write("**Exemples PCI fichier 3 :**", sorted(list(pci_fichier3))[:8])
                            st.write("**Exemples PCI mapping :**",   sorted(list(pci_mapping))[:8])
                        elif len(pci_absents) > 0:
                            st.warning(f"⚠️ {len(pci_absents)} PCI du fichier 3 absents du mapping")
                            st.write("**PCI non trouvés dans le mapping :**",
                                     sorted(list(pci_absents))[:10])

                # ── Jointure left : garde tous les PCI du fichier 3 ──────────
            ...
            def show_table(df_res, key_prefix):
                if df_res is None or df_res.empty:
                    st.warning("⚠️ Aucun résultat — vérifiez les colonnes sélectionnées.")
                    return
                mont_col = df_res.columns[2]
                bal_col  = df_res.columns[3]
                ecar_col = df_res.columns[4]
                k1,k2,k3,k4 = st.columns(4)
                k1.metric("Lignes", len(df_res))
                k2.metric(f"Total {mont_col[:16]}", f"{df_res[mont_col].sum():,.0f}".replace(',',' '))
                k3.metric("Total Balance",           f"{df_res[bal_col].sum():,.0f}".replace(',',' '))
                k4.metric("Écart total",             f"{df_res[ecar_col].sum():,.0f}".replace(',',' '),
                          delta=f"{df_res[ecar_col].sum():,.0f}".replace(',',' '),
                          delta_color="inverse" if df_res[ecar_col].sum() != 0 else "off")
                fc1, fc2 = st.columns(2)
                search      = fc1.text_input("🔍 PCI / PCEC", key=f"{key_prefix}_srch")
                ecarts_only = fc2.checkbox("Écarts ≠ 0 uniquement", key=f"{key_prefix}_ec")
                df_d = df_res.copy()
                if search:
                    df_d = df_d[df_d['PCI'].str.contains(search,case=False,na=False) |
                ...
                with ce2:
                    ...
                    out.seek(0)
                    st.download_button(f"📥 Télécharger Excel", data=out,
                        file_name=f"{key_prefix}_{datetime.now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True, key=f"{key_prefix}_dl")

            # ══════════════════════════════════════════════════════════════════
            # SECTION 1 : Balance + Mapping (config commune, sauvegardée)
            # ══════════════════════════════════════════════════════════════════
            bal_cfg_ok = st.session_state.immos_col_pcec_f1 is not None

            with st.expander("⚙️ Config Balance & Mapping (commun aux 3 tableaux)",
                             expanded=not bal_cfg_ok):
                st.markdown("**Fichier 1 — Balance PCEC**")
                ...
                st.markdown("**Fichier 2 — Mapping PCEC ↔ PCI**")
                c2a, c2b = st.columns(2)
                _pcec_f2 = c2a.selectbox("Colonne PCEC", cols_f2,
                    index=safe_idx(cols_f2, st.session_state.immos_col_pcec_f2,
                                   safe_idx(cols_f2, auto(cols_f2,'PCEC'))),
                    key="cfg_pcec_f2")
                _pci_f2  = c2b.selectbox("Colonne PCI",  cols_f2,
                    index=safe_idx(cols_f2, st.session_state.immos_col_pci_f2,
                                   safe_idx(cols_f2, auto(cols_f2,'PCI'))),
                    key="cfg_pci_f2")

                sv1,sv2,sv3 = st.columns([1,2,1])
                with sv2:
                    if st.button("💾 Sauvegarder Balance & Mapping", type="primary",
                        ...
                        st.session_state.immos_col_pcec_f2  = _pcec_f2
                        st.session_state.immos_col_pci_f2   = _pci_f2
            ...
            if not bal_cfg_ok:
                st.info("💡 Sauvegardez d'abord la config Balance & Mapping ci-dessus.")
            else:
                st.divider()

                # ── TABLEAU 1 ─────────────────────────────────────────────────
                st.markdown("### 🏗️ Tableau 1 — Bilan Immos")
                t1_cfg_ok = st.session_state.immos_t1_pci_col is not None
                with st.expander("⚙️ Colonnes Tableau 1", expanded=not t1_cfg_ok):
                    tb1a, tb1b = st.columns(2)
                    _t1_pci  = tb1a.selectbox("Colonne PCI",    cols_f3,
                        index=safe_idx(cols_f3, st.session_state.immos_t1_pci_col,
                                       safe_idx(cols_f3, auto(cols_f3,'IMMO_COMPTE','IMMO','CPTE_I'))),
                        key="cfg_t1_pci")
                    _t1_mont = tb1b.selectbox("Colonne Montant", cols_f3,
                        index=safe_idx(cols_f3, st.session_state.immos_t1_mont_col,
                                       safe_idx(cols_f3, auto(cols_f3,'COUT','COÛT','COST'))),
                        key="cfg_t1_mont")
                    s1a,s1b,s1c = st.columns([1,2,1])
                    with s1b:
                        if st.button("💾 Sauvegarder Tableau 1", use_container_width=True,
                                      key="btn_save_t1"):
                            st.session_state.immos_t1_pci_col  = _t1_pci
                            st.session_state.immos_t1_mont_col = _t1_mont
                            st.session_state.immos_res_t1      = None
                            st.success("✅ Config T1 sauvegardée")
                            st.rerun()

                if t1_cfg_ok:
                    ca1,ca2,ca3 = st.columns([1,2,1])
                    with ca2:
                        if st.button("🔗 Calculer Tableau 1", type="primary",
                                      use_container_width=True, key="btn_calc_t1"):
                            with st.spinner("Calcul T1..."):
                                st.session_state.immos_res_t1 = build_table(
                                    st.session_state.immos_t1_pci_col,
                                    st.session_state.immos_t1_mont_col,
                                    'Montant Immos (Gestion)',
                                    diag_key='T1')
                    if st.session_state.immos_res_t1 is not None:
                        show_table(st.session_state.immos_res_t1, "T1_Bilan_Immos")

                st.divider()

                # ── TABLEAU 2 ─────────────────────────────────────────────────
                st.markdown("### 📉 Tableau 2 — Dotations")
                t2_cfg_ok = st.session_state.immos_t2_pci_col is not None
                with st.expander("⚙️ Colonnes Tableau 2", expanded=not t2_cfg_ok):
                    tb2a, tb2b = st.columns(2)
                    _t2_pci  = tb2a.selectbox("Colonne PCI",    cols_f3,
                        index=safe_idx(cols_f3, st.session_state.immos_t2_pci_col,
                                       safe_idx(cols_f3, auto(cols_f3,'DOTATION_COMPTE','DOTATION'))),
                        key="cfg_t2_pci")
                    _t2_mont = tb2b.selectbox("Colonne Montant", cols_f3,
                        index=safe_idx(cols_f3, st.session_state.immos_t2_mont_col,
                                       safe_idx(cols_f3, auto(cols_f3,'DOTATION_EX','EXERCICE'))),
                        key="cfg_t2_mont")
                    s2a,s2b,s2c = st.columns([1,2,1])
                    with s2b:
                        if st.button("💾 Sauvegarder Tableau 2", use_container_width=True,
                                      key="btn_save_t2"):
                            st.session_state.immos_t2_pci_col  = _t2_pci
                            st.session_state.immos_t2_mont_col = _t2_mont
                            st.session_state.immos_res_t2      = None
                            st.success("✅ Config T2 sauvegardée")
                            st.rerun()

                if t2_cfg_ok:
                    cb1,cb2,cb3 = st.columns([1,2,1])
                    with cb2:
                        if st.button("🔗 Calculer Tableau 2", type="primary",
                                      use_container_width=True, key="btn_calc_t2"):
                            with st.spinner("Calcul T2..."):
                                st.session_state.immos_res_t2 = build_table(
                                    st.session_state.immos_t2_pci_col,
                                    st.session_state.immos_t2_mont_col,
                                    f'Montant {st.session_state.immos_t2_mont_col}',
                                    diag_key='T2')
                    if st.session_state.immos_res_t2 is not None:
                        show_table(st.session_state.immos_res_t2, "T2_Dotations")

                st.divider()

                # ── TABLEAU 3 ─────────────────────────────────────────────────
                t3_label  = st.session_state.immos_t3_label or "Tableau 3"
                st.markdown(f"### 📋 Tableau 3 — {t3_label}")
                t3_cfg_ok = st.session_state.immos_t3_pci_col is not None
                with st.expander("⚙️ Colonnes Tableau 3", expanded=not t3_cfg_ok):
                    tb3a, tb3b, tb3c = st.columns(3)
                    _t3_pci   = tb3a.selectbox("Colonne PCI",    cols_f3,
                        index=safe_idx(cols_f3, st.session_state.immos_t3_pci_col, 0),
                        key="cfg_t3_pci")
                    _t3_mont  = tb3b.selectbox("Colonne Montant", cols_f3,
                        index=safe_idx(cols_f3, st.session_state.immos_t3_mont_col,
                                       min(1, len(cols_f3)-1)),
                        key="cfg_t3_mont")
                    _t3_label = tb3c.text_input("Nom tableau 3",
                        value=st.session_state.immos_t3_label or "Tableau 3",
                        key="cfg_t3_label")
                    s3a,s3b,s3c = st.columns([1,2,1])
                    with s3b:
                        if st.button("💾 Sauvegarder Tableau 3", use_container_width=True,
                                      key="btn_save_t3"):
                            st.session_state.immos_t3_pci_col  = _t3_pci
                            st.session_state.immos_t3_mont_col = _t3_mont
                            st.session_state.immos_t3_label    = _t3_label
                            st.session_state.immos_res_t3      = None
                            st.success("✅ Config T3 sauvegardée")
                            st.rerun()

                if t3_cfg_ok:
                    cc1,cc2,cc3 = st.columns([1,2,1])
                    with cc2:
                        if st.button(f"🔗 Calculer {t3_label}", type="primary",
                                      use_container_width=True, key="btn_calc_t3"):
                            with st.spinner(f"Calcul T3..."):
                                st.session_state.immos_res_t3 = build_table(
                                    st.session_state.immos_t3_pci_col,
                                    st.session_state.immos_t3_mont_col,
                                    f'Montant {st.session_state.immos_t3_mont_col}',
                                    diag_key='T3')
                    if st.session_state.immos_res_t3 is not None:
                        show_table(st.session_state.immos_res_t3, f"T3_{t3_label}")
