...
# ─── Nettoyage automatique des caches corrompus (tuple au lieu de DataFrame) ─
import pandas as _pd_check
for _key in ["saham_pdm_localites"]:
    _p = _cache_path(_key)
    if os.path.exists(_p):
        try:
            with open(_p, "rb") as _f:
                _obj = pickle.load(_f)
            if not isinstance(_obj, _pd_check.DataFrame):
                os.remove(_p)
        except Exception:
            try:
                os.remove(_p)
...
if 'saham_pdm_localites' not in st.session_state:
    ...
    # Nettoyer si ancien format tuple (df, df_detail)
    if isinstance(cached_pdm, tuple):
        cached_pdm = cached_pdm[0] if cached_pdm and isinstance(cached_pdm[0], pd.DataFrame) else None
...
if 'saham_tableau_f_detail' not in st.session_state:
    cached_tfd = load_cache("saham_tableau_f_detail")
    st.session_state.saham_tableau_f_detail = cached_tfd
...
def normalize_mapping_localites(df):
    ...
    Colonnes : Code Agence | Code Localité | Localité | DRPP
    """
    df = df.copy()
    df = df.dropna(how='all').reset_index(drop=True)
    ...
    if all_unnamed:
        ...
        if found is not None:
            ...
            nb = len(df.columns)
            names = ['Code_Agence', 'Code_Localite', 'Localite', 'DRPP'] + [f'Col_{i}' for i in range(4, nb)]
            df.columns = names[:nb]

    df.columns = df.columns.astype(str).str.strip()
    rename_map = {}
    for col in df.columns:
        if col in ('Code_Agence', 'Code_Localite', 'Localite', 'DRPP'):
            continue
        cl = col.lower().strip()
        ...
        elif cl in ('drpp', 'dr', 'direction', 'direction regionale', 'direction_regionale',
                    'dir reg', 'dir_reg', 'drp', 'region'):
            rename_map[col] = 'DRPP'
    df.rename(columns=rename_map, inplace=True)
    ...
    if 'DRPP' in df.columns:
        df['DRPP'] = df['DRPP'].astype(str).str.strip()
...
def process_saham_pdm(df_main, df_mapping_chapitre, df_mapping_localites, df_bam=None):
    ...
    # Toujours extraire Annee (col 0) et Mois (col 1) par position
    df_annee_mois = df_main.iloc[:, [0, 1]].copy()
    df_annee_mois.columns = ['Annee', 'Mois']
    # Nettoyer Annee et Mois
    df_annee_mois['Annee'] = (df_annee_mois['Annee'].astype(str)
                               .str.strip().str.strip('"')
                               .str.replace(r'[\s\xa0]', '', regex=True)
                               .str.replace(r'\.0$', '', regex=True))
    df_annee_mois['Mois']  = (df_annee_mois['Mois'].astype(str)
                               .str.strip().str.strip('"')
                               .str.replace(r'[\s\xa0]', '', regex=True)
                               .str.replace(r'\.0$', '', regex=True))
    ...
    if 'Code_Agence' in cols and 'Chapitre' in cols and 'Debit' in cols and 'Credit' in cols:
        ...
        # fallback position: 2=Code_Agence, 3=Chapitre, 4=Debit, last=Credit
        df = df_main.iloc[:, [2, 3, 4, -1]].copy()
        df.columns = ['Code_Agence', 'Chapitre', 'Debit', 'Credit']
        df['Debit']  = pd.to_numeric(df['Debit'],  errors='coerce').fillna(0)
        df['Credit'] = pd.to_numeric(df['Credit'], errors='coerce').fillna(0)

    # Attacher Annee et Mois au df principal
    df['Annee'] = df_annee_mois['Annee'].values
    df['Mois']  = df_annee_mois['Mois'].values
    ...
    # Colonnes à joindre depuis le mapping (Localite + DRPP si présente)
    ml_join_cols = ['Code_Agence', 'Localite']
    if 'DRPP' in df_ml.columns:
        ml_join_cols.append('DRPP')
    # Dédupliquer le mapping (garder 1 ligne par Code_Agence)
    df_ml_dedup = df_ml[ml_join_cols].drop_duplicates(subset=['Code_Agence'])

    # ── 7. Jointure avec mapping localités ───────────────────────────────────
    df = df.merge(df_ml_dedup, on='Code_Agence', how='left')
    df_unknown = df[df['Localite'].isna()]
    nb_unknown = len(df_unknown['Code_Agence'].unique())
    df = df[df['Localite'].notna()].copy()

    # ── 8. Agrégation PRINCIPALE par Localité seule (pour PDM correct) ───────
    # ⚠️ L'agrégation PDM doit TOUJOURS se faire par Localité uniquement,
    # sinon les montants Saham sont fragmentés par mois → PDM divisée par nb mois
    has_annee = 'Annee' in df.columns
    has_mois  = 'Mois'  in df.columns
    has_drpp  = 'DRPP'  in df.columns

    # Colonnes de base pour le tableau F (Annee, Mois, Localite, DRPP)
    grp_cols_f = []
    if has_annee: grp_cols_f.append('Annee')
    if has_mois:  grp_cols_f.append('Mois')
    grp_cols_f.append('Localite')
    if has_drpp:  grp_cols_f.append('DRPP')

    # Agrégation par Localité seule pour PDM
    grp_pdm = ['Localite']
    if has_drpp: grp_pdm.append('DRPP')

    df_credits_pdm = (df[df['Is_Credit']]
                      .groupby(grp_pdm)['Montant_Abs'].sum()
                      .reset_index()
                      .rename(columns={'Montant_Abs': 'Credits_Saham'}))

    df_depots_pdm = (df[df['Is_Depot']]
                     .groupby(grp_pdm)['Montant_Abs'].sum()
                     .reset_index()
                     .rename(columns={'Montant_Abs': 'Depots_Saham'}))

    df_result = df_credits_pdm.merge(df_depots_pdm, on=grp_pdm, how='outer').fillna(0)
    df_result = df_result.sort_values(grp_pdm).reset_index(drop=True)

    # ── 8b. Agrégation Tableau F par Annee+Mois+Localite+DRPP ────────────────
    df_f_detail = None
    if has_annee or has_mois:
        df_credits_f = (df[df['Is_Credit']]
                        .groupby(grp_cols_f)['Montant_Abs'].sum()
                        .reset_index()
                        .rename(columns={'Montant_Abs': 'Credits_Saham_F'}))

        df_depots_f = (df[df['Is_Depot']]
                       .groupby(grp_cols_f)['Montant_Abs'].sum()
                       .reset_index()
                       .rename(columns={'Montant_Abs': 'Depots_Saham_F'}))

        df_f_detail = df_credits_f.merge(df_depots_f, on=grp_cols_f, how='outer').fillna(0)
        df_f_detail = df_f_detail.sort_values(grp_cols_f).reset_index(drop=True)
        # Stocker aussi dans attrs pour compatibilité
        df_result.attrs['tableau_f_detail'] = df_f_detail
        df_result.attrs['tableau_f_cols']   = grp_cols_f

    # Sauvegarder stats de jointure
    df_result.attrs['nb_agences_non_mappes'] = nb_unknown

    # ── 9. Calcul PDM vs BAM (sur agrégation par Localité seule) ─────────────
    if df_bam is not None:
        ...
        if 'Localite' in bam_norm.columns:
            ...
            if agg_cols:
                # ── Agréger BAM par Localité normalisée (évite les doublons) ──
                bam_norm['_merge_key'] = bam_norm['Localite'].astype(str).str.upper().str.strip()
                bam_agg = (bam_norm
                           .groupby('_merge_key')[list(agg_cols.keys())]
                           .sum()
                           .reset_index())
                # Une seule ligne par clé normalisée → pas de doublon possible
                ...
                # ⚠️ Saham en DH bruts, BAM en milliers de DH → BAM × 1000
                ...
                # Propager BAM dans le détail Tableau F
                # Si BAM contient Annee+mois, joindre par Annee+Mois+Localite
                # pour avoir un montant BAM correct par période
                if df_f_detail is not None:
                    df_fd = df_f_detail.copy()
                    df_fd['_loc_key'] = df_fd['Localite'].str.upper().str.strip()

                    bam_has_annee = 'Annee' in bam_norm.columns
                    bam_has_mois  = 'mois'  in bam_norm.columns
                    fd_has_annee  = 'Annee' in df_fd.columns
                    fd_has_mois   = 'Mois'  in df_fd.columns

                    if bam_has_annee and bam_has_mois and fd_has_annee and fd_has_mois:
                        # Agrégation BAM par Annee+Mois+Localite
                        bam_norm['_loc_key'] = bam_norm['Localite'].astype(str).str.upper().str.strip()
                        bam_norm['_annee']   = bam_norm['Annee'].astype(str).str.strip()
                        bam_norm['_mois']    = bam_norm['mois'].apply(
                            lambda m: str(int(float(m)))
                            if str(m).replace('.0','').strip().isdigit() else str(m)
                        )
                        bam_agg_f = (bam_norm
                                     .groupby(['_annee', '_mois', '_loc_key'])[list(agg_cols.keys())]
                                     .sum()
                                     .reset_index())

                        df_fd['_annee'] = df_fd['Annee'].astype(str).str.strip()
                        df_fd['_mois']  = df_fd['Mois'].apply(
                            lambda m: str(int(float(m)))
                            if str(m).replace('.0','').strip().isdigit() else str(m)
                        )
                        df_fd = df_fd.merge(
                            bam_agg_f[['_annee', '_mois', '_loc_key'] + list(agg_cols.keys())],
                            on=['_annee', '_mois', '_loc_key'], how='left'
                        ).drop(columns=['_annee', '_mois', '_loc_key'], errors='ignore')
                    else:
                        # Fallback : propagation par Localite uniquement
                        df_fd = df_fd.merge(
                            bam_agg[['_merge_key'] + list(agg_cols.keys())],
                            left_on='_loc_key', right_on='_merge_key', how='left'
                        ).drop(columns=['_merge_key', '_loc_key'], errors='ignore')

                    df_fd[list(agg_cols.keys())] = df_fd[list(agg_cols.keys())].fillna(0)
                    for _c in list(agg_cols.keys()):
                        df_fd[_c] = pd.to_numeric(df_fd[_c], errors='coerce').fillna(0)

                    if 'Montant_Credits' in df_fd.columns:
                        df_fd['PDM_Credits_F'] = np.where(
                            df_fd['Montant_Credits'] > 0,
                            df_fd['Credits_Saham_F'] / (df_fd['Montant_Credits'] * 1000) * 100, 0
                        )
                    if 'Montant_Depots' in df_fd.columns:
                        df_fd['PDM_Depots_F'] = np.where(
                            df_fd['Montant_Depots'] > 0,
                            df_fd['Depots_Saham_F'] / (df_fd['Montant_Depots'] * 1000) * 100, 0
                        )
                    df_f_detail = df_fd
                    df_result.attrs['tableau_f_detail'] = df_f_detail

    return df_result, df_f_detail




def viz_pdm_saham_localites(df_pdm):
    # ── Sécurité : s'assurer que df_pdm est bien un DataFrame ────────────────
    if isinstance(df_pdm, tuple):
        df_pdm = df_pdm[0] if df_pdm and isinstance(df_pdm[0], pd.DataFrame) else pd.DataFrame()
    if not isinstance(df_pdm, pd.DataFrame) or df_pdm.empty:
        st.warning("⚠️ Données PDM invalides. Relancez le calcul PDM.")
        return

    # ── Sauvegarder attrs avant toute manipulation ────────────────────────────
    # ── Vider les attrs AVANT toute opération pandas (évite ValueError ambiguous) ──
    # df_pdm.attrs peut contenir tableau_f_detail (un DataFrame) → pandas plante
    # sur nlargest/concat/merge quand il compare attrs
    _attrs_saved = {}   # on ne restaure PAS les attrs pour éviter le bug
    df_pdm.attrs = {}   # vider immédiatement

    # ── Dédupliquer : garantir 1 ligne par Localité (évite doublons BAM) ──────
    if 'Localite' in df_pdm.columns:

        # ── Reconstruire un DataFrame propre colonne par colonne ─────────────
        # (évite les colonnes imbriquées/Series issues du merge BAM)
        _clean_cols = ['Localite', 'DRPP',
                       'Credits_Saham', 'Depots_Saham',
                       'Montant_Credits', 'Montant_Depots',
                       'PDM_Credits', 'PDM_Depots']
        _present = [c for c in _clean_cols if c in df_pdm.columns]
        _rebuild = {}
        for c in _present:
            col_data = df_pdm[c]
            # Si la colonne est un DataFrame (merge dupliqué), prendre la 1ère
            ...
            if isinstance(col_data, pd.DataFrame):
                col_data = col_data.iloc[:, 0]
            _rebuild[c] = col_data.values
        df_pdm = pd.DataFrame(_rebuild)

        # Forcer numériques
        for col in ['Credits_Saham', 'Depots_Saham',
                    'Montant_Credits', 'Montant_Depots',
                    'PDM_Credits', 'PDM_Depots']:
            if col in df_pdm.columns:
                df_pdm[col] = pd.to_numeric(df_pdm[col], errors='coerce').fillna(0)

        grp = ['Localite']
        if 'DRPP' in df_pdm.columns:
            grp.append('DRPP')

        sum_cols   = [c for c in ['Credits_Saham', 'Depots_Saham']     if c in df_pdm.columns]
        first_cols = [c for c in ['Montant_Credits', 'Montant_Depots'] if c in df_pdm.columns]

        if sum_cols or first_cols:
            # Agrégation ligne par ligne — 100% safe
            rows = []
            for key, gdf in df_pdm.groupby(grp, sort=False):
                if not isinstance(key, tuple):
                    key = (key,)
                row = {c: k for c, k in zip(grp, key)}
                for c in sum_cols:
                    row[c] = float(pd.to_numeric(gdf[c], errors='coerce').fillna(0).sum())
                for c in first_cols:
                    row[c] = float(pd.to_numeric(gdf[c], errors='coerce').fillna(0).iloc[0])
                rows.append(row)
            df_pdm = pd.DataFrame(rows)

            # Recalculer PDM après déduplification
            if 'Montant_Credits' in df_pdm.columns and 'Credits_Saham' in df_pdm.columns:
                mc = pd.to_numeric(df_pdm['Montant_Credits'], errors='coerce').fillna(0)
                cs = pd.to_numeric(df_pdm['Credits_Saham'],  errors='coerce').fillna(0)
                df_pdm['PDM_Credits'] = np.where(mc > 0, cs / (mc * 1000) * 100, 0.0)
            if 'Montant_Depots' in df_pdm.columns and 'Depots_Saham' in df_pdm.columns:
                md = pd.to_numeric(df_pdm['Montant_Depots'], errors='coerce').fillna(0)
                ds = pd.to_numeric(df_pdm['Depots_Saham'],  errors='coerce').fillna(0)
                df_pdm['PDM_Depots'] = np.where(md > 0, ds / (md * 1000) * 100, 0.0)

        df_pdm.attrs = _attrs_saved
    ...
    # ⚠️ Saham en DH bruts, BAM en milliers de DH → × 1000 pour comparer
    ...
    # ONGLETS DIRECTEUR
    # ═══════════════════════════════════════════════════════════════════
    tab_vue, tab_dep, tab_cred, tab_comp, tab_heatmap, tab_unified = st.tabs([
        "🏆 Vue Directeur",
        "🏦 PDM Dépôts",
        "💳 PDM Crédits",
        "⚖️ Saham vs BAM",
        "🔥 Carte de Chaleur",
        "📋 Tableau PDM Détaillé"
    ])
        ...
        if not has_bam_credits and not has_bam_depots:
            ...
            if type_c == "Scatter PDM" and has_pdm_credits and has_pdm_depots:
                ...
                if col_s:
                    ...
                    fig_cp.add_trace(go.Bar(
                        # Saham en DH bruts → /1e9 pour afficher en Md DH
    ...
    # ─── TAB TABLEAU PDM DÉTAILLÉ (UNIFIÉ) ──────────────────────────────
    with tab_unified:
        st.subheader("📋 Tableau PDM Détaillé — Historique complet")

        mois_noms_u = {1:'Janvier',2:'Février',3:'Mars',4:'Avril',5:'Mai',6:'Juin',
                       7:'Juillet',8:'Août',9:'Septembre',10:'Octobre',11:'Novembre',12:'Décembre'}

        # ── Source : reconstruire depuis les données brutes complètes ────────────
        # On utilise saham_credits_depots_raw (toutes périodes) + mappings + BAM complet
        raw_u   = st.session_state.get('saham_credits_depots_raw', None)
        mc_u    = st.session_state.get('saham_mapping_chapitre', None)
        mla_u   = st.session_state.get('saham_mapping_localites_agences', None)
        bam_u   = st.session_state.get('combined_data_bam', None)

        # Fallback : si pas de données brutes, utiliser tableau_f_detail déjà calculé
        _tmp_hist = st.session_state.get('saham_tableau_f_detail', None)
        if _tmp_hist is None or not isinstance(_tmp_hist, pd.DataFrame):
            _tmp_hist = df_pdm.attrs.get('tableau_f_detail', None)
        df_hist = _tmp_hist if isinstance(_tmp_hist, pd.DataFrame) and not _tmp_hist.empty else None

        if raw_u is not None and mc_u is not None and mla_u is not None:
            # Recalculer sur TOUTES les données sans filtre de période
            with st.spinner("⏳ Chargement de l'historique complet..."):
                try:
                    bam_full = normalize_bam_columns(bam_u.copy()) if bam_u is not None else None
                    if bam_full is not None:
                        bam_full = clean_numeric_columns(bam_full)
                    _, df_hist = process_saham_pdm(raw_u, mc_u, mla_u, bam_full)
                except Exception as _e_hist:
                    st.warning(f"⚠️ Recalcul historique : {_e_hist}. Utilisation du cache existant.")

        if df_hist is None:
            st.warning("⚠️ Aucune donnée disponible. Importez vos données Saham et lancez le calcul PDM.")
        else:
            # ── FILTRES ──────────────────────────────────────────────────────────
            ...
            if 'Annee' in df_hist.columns:
                annees_u = sorted(df_hist['Annee'].dropna().astype(str).str.strip()
                                  .str.replace(r'\.0$', '', regex=True).unique().tolist())

            annee_u_sel = flt1.selectbox(
                "Année",
                ["Toutes"] + annees_u,
                index=len(annees_u) if annees_u else 0,
                key="unified_annee_sel"
            )

            # Mois disponibles selon l'année
            mois_u = []
            if 'Mois' in df_hist.columns:
                src_m = df_hist if annee_u_sel == "Toutes" else df_hist[
                    df_hist['Annee'].astype(str).str.strip().str.replace(r'\.0$','',regex=True) == annee_u_sel]
                mois_u = sorted([
                    int(float(m)) for m in src_m['Mois'].dropna().unique()
                    if str(m).replace('.0','').strip().isdigit() and 1 <= int(float(m)) <= 12
                ])

            mois_u_options = ["Tous"] + [str(m) for m in mois_u]
            mois_u_sel = flt2.selectbox(
                "Mois",
                mois_u_options,
                format_func=lambda x: "Tous" if x == "Tous" else mois_noms_u.get(int(x), x),
                index=len(mois_u_options) - 1 if len(mois_u_options) > 1 else 0,
                key="unified_mois_sel"
            )

            srch_u = flt3.text_input("🔍 Rechercher une localité", key="unified_srch")

            st.divider()

            # ── FILTRAGE ─────────────────────────────────────────────────────────
            dfu = df_hist.copy()
            if annee_u_sel != "Toutes" and 'Annee' in dfu.columns:
                dfu = dfu[dfu['Annee'].astype(str).str.strip()
                          .str.replace(r'\.0$','',regex=True) == annee_u_sel]
            if mois_u_sel != "Tous" and 'Mois' in dfu.columns:
                dfu = dfu[dfu['Mois'].apply(
                    lambda m: str(int(float(m))) == mois_u_sel
                    if str(m).replace('.0','').strip().isdigit() else False)]
            if srch_u and 'Localite' in dfu.columns:
                dfu = dfu[dfu['Localite'].astype(str).str.contains(srch_u, case=False, na=False)]

            # ── Agréger : 1 ligne par Annee+Mois+Localite+DRPP ──────────────────
            # (plusieurs agences Saham → somme par localité)
            grp_u = [c for c in ['Annee','Mois','Localite','DRPP'] if c in dfu.columns]
            sum_cols_u   = [c for c in ['Credits_Saham_F','Depots_Saham_F'] if c in dfu.columns]
            first_cols_u = [c for c in ['Montant_Credits','Montant_Depots'] if c in dfu.columns]

            if grp_u and (sum_cols_u or first_cols_u):
                try:
                    agg_u = {c: 'sum' for c in sum_cols_u}
                    agg_u.update({c: 'first' for c in first_cols_u})
                    dfu = dfu.groupby(grp_u, as_index=False).agg(agg_u)
                    # Recalculer PDM après agrégation
                    if 'Montant_Credits' in dfu.columns and 'Credits_Saham_F' in dfu.columns:
                        mc_ = pd.to_numeric(dfu['Montant_Credits'], errors='coerce').fillna(0)
                        cs_ = pd.to_numeric(dfu['Credits_Saham_F'], errors='coerce').fillna(0)
                        dfu['PDM_Credits_F'] = np.where(mc_ > 0, cs_ / (mc_ * 1000) * 100, 0.0)
                    if 'Montant_Depots' in dfu.columns and 'Depots_Saham_F' in dfu.columns:
                        md_ = pd.to_numeric(dfu['Montant_Depots'], errors='coerce').fillna(0)
                        ds_ = pd.to_numeric(dfu['Depots_Saham_F'], errors='coerce').fillna(0)
                        dfu['PDM_Depots_F'] = np.where(md_ > 0, ds_ / (md_ * 1000) * 100, 0.0)
                except Exception:
                    pass

            st.info(f"📊 **{len(dfu):,} lignes** — {dfu['Localite'].nunique() if 'Localite' in dfu.columns else 0} localités")

            # ── COLONNES SOURCES ─────────────────────────────────────────────────
            dep_s_u  = 'Depots_Saham_F'  if 'Depots_Saham_F'  in dfu.columns else None
            cred_s_u = 'Credits_Saham_F' if 'Credits_Saham_F' in dfu.columns else None
            dep_b_u  = 'Montant_Depots'  if 'Montant_Depots'  in dfu.columns else None
            cred_b_u = 'Montant_Credits' if 'Montant_Credits' in dfu.columns else None
            pdm_d_u  = 'PDM_Depots_F'    if 'PDM_Depots_F'    in dfu.columns else None
            pdm_c_u  = 'PDM_Credits_F'   if 'PDM_Credits_F'   in dfu.columns else None

            # ── TABLEAU AFFICHÉ ──────────────────────────────────────────────────
            disp_u = pd.DataFrame()
            if 'Annee' in dfu.columns:
                disp_u['Année'] = dfu['Annee'].astype(str).str.replace(r'\.0$','',regex=True).values
            if 'Mois' in dfu.columns:
                disp_u['Mois']  = dfu['Mois'].apply(
                    lambda m: mois_noms_u.get(int(float(m)), str(m))
                    if str(m).replace('.0','').strip().isdigit() else str(m))
            if 'Localite' in dfu.columns:
                disp_u['Localité'] = dfu['Localite'].values
            if 'DRPP' in dfu.columns:
                disp_u['DRPP'] = dfu['DRPP'].values
            if dep_s_u:
                disp_u['Dépôts Saham (Md DH)']  = pd.to_numeric(dfu[dep_s_u], errors='coerce').fillna(0).div(1e9).round(4)
            if dep_b_u:
                disp_u['Dépôts BAM (Md DH)']    = pd.to_numeric(dfu[dep_b_u], errors='coerce').fillna(0).div(1e6).round(4)
            if pdm_d_u:
                disp_u['PDM Dépôts (%)']         = pd.to_numeric(dfu[pdm_d_u], errors='coerce').fillna(0).round(3)
            if cred_s_u:
                disp_u['Crédits Saham (Md DH)'] = pd.to_numeric(dfu[cred_s_u], errors='coerce').fillna(0).div(1e9).round(4)
            if cred_b_u:
                disp_u['Crédits BAM (Md DH)']   = pd.to_numeric(dfu[cred_b_u], errors='coerce').fillna(0).div(1e6).round(4)
            if pdm_c_u:
                disp_u['PDM Crédits (%)']        = pd.to_numeric(dfu[pdm_c_u], errors='coerce').fillna(0).round(3)

            # Formatage conditionnel pour les colonnes PDM
            def _style_pdm(val):
                try:
                    v = float(val)
            ...
            pdm_display_cols = [c for c in ['PDM Dépôts (%)','PDM Crédits (%)'] if c in disp_u.columns]
            if pdm_display_cols:
                styled = disp_u.style.applymap(_style_pdm, subset=pdm_display_cols)
                st.dataframe(styled, use_container_width=True, hide_index=True, height=540)
            else:
                st.dataframe(disp_u, use_container_width=True, hide_index=True, height=540)

            st.divider()

            # ── EXPORT EXCEL ─────────────────────────────────────────────────────
            import openpyxl as _opxl_u
            ...
            pct_u = set()
            if 'Annee' in dfu.columns:
                exp_u['Année'] = dfu['Annee'].astype(str).str.replace(r'\.0$','',regex=True).values
            if 'Mois' in dfu.columns:
                exp_u['Mois'] = dfu['Mois'].apply(
                    lambda m: mois_noms_u.get(int(float(m)), str(m))
                    if str(m).replace('.0','').strip().isdigit() else str(m))
            if 'Localite' in dfu.columns:
                exp_u['Localité'] = dfu['Localite'].values
            if 'DRPP' in dfu.columns:
                exp_u['DRPP'] = dfu['DRPP'].values
            if dep_s_u:
                exp_u['Dépôts Saham (Md DH)']  = pd.to_numeric(dfu[dep_s_u], errors='coerce').fillna(0).div(1e9).round(6)
                num_u.add('Dépôts Saham (Md DH)')
            if dep_b_u:
                exp_u['Dépôts BAM (Md DH)']    = pd.to_numeric(dfu[dep_b_u], errors='coerce').fillna(0).div(1e6).round(6)
                num_u.add('Dépôts BAM (Md DH)')
            if pdm_d_u:
                exp_u['PDM Dépôts (%)']         = pd.to_numeric(dfu[pdm_d_u], errors='coerce').fillna(0).round(4)
                pct_u.add('PDM Dépôts (%)')
            if cred_s_u:
                exp_u['Crédits Saham (Md DH)'] = pd.to_numeric(dfu[cred_s_u], errors='coerce').fillna(0).div(1e9).round(6)
                num_u.add('Crédits Saham (Md DH)')
            if cred_b_u:
                exp_u['Crédits BAM (Md DH)']   = pd.to_numeric(dfu[cred_b_u], errors='coerce').fillna(0).div(1e6).round(6)
                num_u.add('Crédits BAM (Md DH)')
            if pdm_c_u:
                exp_u['PDM Crédits (%)']        = pd.to_numeric(dfu[pdm_c_u], errors='coerce').fillna(0).round(4)
                pct_u.add('PDM Crédits (%)')

            # Label du fichier téléchargé
            lbl_u = (
                f"{annee_u_sel}_{mois_noms_u.get(int(mois_u_sel), mois_u_sel)}"
                if annee_u_sel != "Toutes" and mois_u_sel != "Tous"
                else annee_u_sel if annee_u_sel != "Toutes"
                else "Historique_complet"
            )

            out_u = BytesIO()
            with pd.ExcelWriter(out_u, engine='openpyxl') as w_u:
                exp_u.to_excel(w_u, index=False, sheet_name='PDM_Historique')
                ws_u = w_u.sheets['PDM_Historique']

                # ── Style en-tête ────────────────────────────────────────
                hdr_fill   = PatternFill("solid", fgColor="1a4d3e")
                hdr_font   = Font(bold=True, color="FFFFFF", size=11)
                hdr_align  = Alignment(horizontal='center', vertical='center', wrap_text=True)
                thin_side  = Side(style='thin', color='CCCCCC')
                thin_border = Border(left=thin_side, right=thin_side, top=thin_side, bottom=thin_side)

                hdr_row = [cell.value for cell in ws_u[1]]
                for ci, cn in enumerate(hdr_row, start=1):
                    cell = ws_u.cell(row=1, column=ci)
                    cell.fill   = hdr_fill
                    cell.font   = hdr_font
                    ...
                    ws_u.column_dimensions[col_letter].width = max(18, len(str(cn or '')) + 4)

                # ── Format cellules données ──────────────────────────────
                green_fill  = PatternFill("solid", fgColor="d4edda")
                yellow_fill = PatternFill("solid", fgColor="fff3cd")
                red_fill    = PatternFill("solid", fgColor="f8d7da")

                for row in ws_u.iter_rows(min_row=2, max_row=ws_u.max_row):
                    for cell in row:
                        cell.border = thin_border
                        cell.alignment = Alignment(horizontal='center', vertical='center')
                        cn = hdr_row[cell.column - 1] if cell.column - 1 < len(hdr_row) else ''
                        if cn in num_u and cell.value is not None:
                            try:
                                cell.value = float(cell.value)
                                cell.number_format = '#,##0.000000'
                            except: pass
                        elif cn in pct_u and cell.value is not None:
                            try:
                                v = float(cell.value)
                                cell.value = v
                                cell.number_format = '0.0000"%"'
                                if v >= 5:   cell.fill = green_fill
                                elif v >= 2: cell.fill = yellow_fill
                                else:        cell.fill = red_fill
                            except: pass

                # Figer la première ligne
                ws_u.freeze_panes = 'A2'
                ws_u.row_dimensions[1].height = 35

            c1, c2, c3 = st.columns([1, 2, 1])
            with c2:
                st.download_button(
                    f"📥 Télécharger — {lbl_u} (Excel)",
                    data=out_u.getvalue(),
                    file_name=f"Saham_PDM_{lbl_u}_{datetime.now().strftime('%Y%m%d')}.xlsx",
            ...
            # ── EXPORT HISTORIQUE — DERNIER MOIS (mois 12) PAR ANNÉE ─────────────
            st.markdown("#### 📅 Télécharger l'historique annuel (Décembre de chaque année)")
            st.caption("Extrait uniquement le mois de Décembre (mois 12) pour chaque année disponible — vue évolution annuelle.")

            if isinstance(df_hist, pd.DataFrame) and not df_hist.empty and 'Mois' in df_hist.columns and 'Annee' in df_hist.columns:

                # Filtrer mois 12 uniquement
                df_dec = df_hist.copy()
                df_dec = df_dec[df_dec['Mois'].apply(
                    lambda m: str(m).replace('.0','').strip() == '12'
                    if str(m).replace('.0','').strip().isdigit() else False
                )]

                if df_dec.empty:
                    st.warning("⚠️ Aucune donnée de Décembre (mois 12) trouvée dans l'historique.")
                else:
                    # Agréger : 1 ligne par Annee+Localite+DRPP
                    grp_dec = [c for c in ['Annee','Localite','DRPP'] if c in df_dec.columns]
                    sum_dec   = [c for c in ['Credits_Saham_F','Depots_Saham_F']  if c in df_dec.columns]
                    first_dec = [c for c in ['Montant_Credits','Montant_Depots']  if c in df_dec.columns]
                    if grp_dec and (sum_dec or first_dec):
                        try:
                            agg_dec = {c:'sum' for c in sum_dec}
                            agg_dec.update({c:'first' for c in first_dec})
                            df_dec = df_dec.groupby(grp_dec, as_index=False).agg(agg_dec)
                            # Recalculer PDM
                            if 'Montant_Credits' in df_dec.columns and 'Credits_Saham_F' in df_dec.columns:
                                mc_d = pd.to_numeric(df_dec['Montant_Credits'], errors='coerce').fillna(0)
                                cs_d = pd.to_numeric(df_dec['Credits_Saham_F'], errors='coerce').fillna(0)
                                df_dec['PDM_Credits_F'] = np.where(mc_d > 0, cs_d / (mc_d * 1000) * 100, 0.0)
                            if 'Montant_Depots' in df_dec.columns and 'Depots_Saham_F' in df_dec.columns:
                                md_d = pd.to_numeric(df_dec['Montant_Depots'], errors='coerce').fillna(0)
                                ds_d = pd.to_numeric(df_dec['Depots_Saham_F'], errors='coerce').fillna(0)
                                df_dec['PDM_Depots_F'] = np.where(md_d > 0, ds_d / (md_d * 1000) * 100, 0.0)
                        except Exception:
                            pass

                    annees_dec = sorted(df_dec['Annee'].dropna().astype(str)
                                        .str.replace(r'\.0$','',regex=True).unique().tolist())
                    st.info(f"📊 **{len(df_dec):,} lignes** — {len(annees_dec)} années disponibles : {', '.join(annees_dec)}")

                    # Construire le DataFrame export Décembre
                    dep_s_d  = 'Depots_Saham_F'  if 'Depots_Saham_F'  in df_dec.columns else None
                    cred_s_d = 'Credits_Saham_F' if 'Credits_Saham_F' in df_dec.columns else None
                    dep_b_d  = 'Montant_Depots'  if 'Montant_Depots'  in df_dec.columns else None
                    cred_b_d = 'Montant_Credits' if 'Montant_Credits' in df_dec.columns else None
                    pdm_d_d  = 'PDM_Depots_F'    if 'PDM_Depots_F'    in df_dec.columns else None
                    pdm_c_d  = 'PDM_Credits_F'   if 'PDM_Credits_F'   in df_dec.columns else None

                    exp_dec = pd.DataFrame()
                    num_dec = set()
                    pct_dec = set()
                    exp_dec['Année']    = df_dec['Annee'].astype(str).str.replace(r'\.0$','',regex=True).values
                    exp_dec['Mois']     = 'Décembre'
                    if 'Localite' in df_dec.columns:
                        exp_dec['Localité'] = df_dec['Localite'].values
                    if 'DRPP' in df_dec.columns:
                        exp_dec['DRPP']     = df_dec['DRPP'].values
                    if dep_s_d:
                        exp_dec['Dépôts Saham (Md DH)']  = pd.to_numeric(df_dec[dep_s_d], errors='coerce').fillna(0).div(1e9).round(6)
                        num_dec.add('Dépôts Saham (Md DH)')
                    if dep_b_d:
                        exp_dec['Dépôts BAM (Md DH)']    = pd.to_numeric(df_dec[dep_b_d], errors='coerce').fillna(0).div(1e6).round(6)
                        num_dec.add('Dépôts BAM (Md DH)')
                    if pdm_d_d:
                        exp_dec['PDM Dépôts (%)']         = pd.to_numeric(df_dec[pdm_d_d], errors='coerce').fillna(0).round(4)
                        pct_dec.add('PDM Dépôts (%)')
                    if cred_s_d:
                        exp_dec['Crédits Saham (Md DH)'] = pd.to_numeric(df_dec[cred_s_d], errors='coerce').fillna(0).div(1e9).round(6)
                        num_dec.add('Crédits Saham (Md DH)')
                    if cred_b_d:
                        exp_dec['Crédits BAM (Md DH)']   = pd.to_numeric(df_dec[cred_b_d], errors='coerce').fillna(0).div(1e6).round(6)
                        num_dec.add('Crédits BAM (Md DH)')
                    if pdm_c_d:
                        exp_dec['PDM Crédits (%)']        = pd.to_numeric(df_dec[pdm_c_d], errors='coerce').fillna(0).round(4)
                        pct_dec.add('PDM Crédits (%)')

                    # Trier par Année puis Localité
                    exp_dec = exp_dec.sort_values(['Année','Localité'] if 'Localité' in exp_dec.columns else ['Année']).reset_index(drop=True)

                    # Générer Excel avec un onglet par année
                    out_dec = BytesIO()
                    with pd.ExcelWriter(out_dec, engine='openpyxl') as w_dec:
                        # Onglet global toutes années
                        exp_dec.to_excel(w_dec, index=False, sheet_name='Historique_Décembre')

                        # Un onglet par année
                        for ann in annees_dec:
                            df_ann = exp_dec[exp_dec['Année'] == ann]
                            sheet_name = f"Déc_{ann}"[:31]
                            df_ann.to_excel(w_dec, index=False, sheet_name=sheet_name)

                        # Styler tous les onglets
                        from openpyxl.styles import PatternFill as PF2, Font as F2, Alignment as A2, Border as B2, Side as S2
                        from openpyxl.utils import get_column_letter as gcl2
                        hdr_fill2  = PF2("solid", fgColor="1a4d3e")
                        hdr_font2  = F2(bold=True, color="FFFFFF", size=11)
                        thin2      = S2(style='thin', color='CCCCCC')
                        brd2       = B2(left=thin2, right=thin2, top=thin2, bottom=thin2)
                        g_fill2    = PF2("solid", fgColor="d4edda")
                        y_fill2    = PF2("solid", fgColor="fff3cd")
                        r_fill2    = PF2("solid", fgColor="f8d7da")

                        for ws_name in w_dec.sheets:
                            ws2 = w_dec.sheets[ws_name]
                            hdr2 = [c.value for c in ws2[1]]
                            for ci2, cn2 in enumerate(hdr2, start=1):
                                cell2 = ws2.cell(row=1, column=ci2)
                                cell2.fill = hdr_fill2
                                cell2.font = hdr_font2
                                cell2.alignment = A2(horizontal='center', vertical='center', wrap_text=True)
                                cell2.border = brd2
                                ws2.column_dimensions[gcl2(ci2)].width = max(18, len(str(cn2 or '')) + 4)
                            for row2 in ws2.iter_rows(min_row=2, max_row=ws2.max_row):
                                for cell2 in row2:
                                    cell2.border = brd2
                                    cell2.alignment = A2(horizontal='center', vertical='center')
                                    cn2 = hdr2[cell2.column-1] if cell2.column-1 < len(hdr2) else ''
                                    if cn2 in num_dec and cell2.value is not None:
                                        try:
                                            cell2.value = float(cell2.value)
                                            cell2.number_format = '#,##0.000000'
                                        except: pass
                                    elif cn2 in pct_dec and cell2.value is not None:
                                        try:
                                            v2 = float(cell2.value)
                                            cell2.value = v2
                                            cell2.number_format = '0.0000"%"'
                                            if v2 >= 5:   cell2.fill = g_fill2
                                            elif v2 >= 2: cell2.fill = y_fill2
                                            else:         cell2.fill = r_fill2
                                        except: pass
                            ws2.freeze_panes = 'A2'
                    ...
                    with cd2:
                        st.download_button(
                            f"📥 Télécharger Historique Décembre {' / '.join(annees_dec)} (Excel)",
                            data=out_dec.getvalue(),
                            file_name=f"Saham_PDM_Historique_Decembre_{datetime.now().strftime('%Y%m%d')}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True,
                            key="dl_historique_dec"
                        )
            else:
                st.info("ℹ️ Chargez vos données pour accéder à l'historique annuel.")
...
def create_saham_visualizations():
    ...
    # ── Nettoyer le cache si format tuple corrompu ───────────────────────────
    pdm_data = st.session_state.saham_pdm_localites

    # Si c'est un tuple (ancienne version), extraire le DataFrame et corriger le cache
    if isinstance(pdm_data, tuple):
        pdm_data = pdm_data[0] if pdm_data and isinstance(pdm_data[0], pd.DataFrame) else None
        st.session_state.saham_pdm_localites = pdm_data
        if pdm_data is not None:
            save_cache("saham_pdm_localites", pdm_data)
        else:
            clear_cache("saham_pdm_localites")

    # Vérification finale
    if not isinstance(pdm_data, pd.DataFrame):
        pdm_data = None
        st.session_state.saham_pdm_localites = None
        clear_cache("saham_pdm_localites")

    if pdm_data is not None and not pdm_data.empty:
        viz_pdm_saham_localites(pdm_data)
    else:
        st.markdown("""
    ...
    with tab1:
        ...
        for annee in sorted(df_derniers_mois['Annee'].unique()):
            ...
            data_tableau.append({
                ...
                'Nb Lignes': nb_lignes,
                'Total Dépôts': f"{total_depots/1e9:.2f} Md",
                'Total Crédits': f"{total_credits/1e9:.2f} Md"
            })
...
def balance_module():
    ...
    with tab4:
        ...
        # ── Session state : config + résultats persistants ───────────────────
        for _k in [
            # Balance (commune)
            ...
            # Tableau 1
            't1_label',
            't1_map_pci', 't1_map_pcec',
            't1_pci_col', 't1_mont_col',
            'immos_res_t1',
            # Tableau 2
            't2_label',
            't2_map_pci', 't2_map_pcec',
            't2_pci_col', 't2_mont_col',
            'immos_res_t2',
            # Tableau 3
            't3_label',
            't3_map_pci', 't3_map_pcec',
            't3_pci_col', 't3_mont_col',
            'immos_res_t3',
        ]:
            if _k not in st.session_state:
                st.session_state[_k] = None
        ...
        if not all_loaded:
            ...
            def auto(lst, *kws):
                for kw in kws:
                    h = next((c for c in lst if kw.upper() in str(c).upper()), None)
                    if h: return h
                return lst[0]
            ...
            # ── Balance PCEC (commune) ────────────────────────────────────────
            def get_balance_df():
                col_pcec = st.session_state.immos_col_pcec_f1
                mode     = st.session_state.immos_mode_mont
                if col_pcec is None: return None
                df_b = df_bal[[col_pcec]].copy()
                df_b['PCEC'] = clean_code(df_b[col_pcec])
                if mode == 'Créditeur − Débiteur':
                    df_b['Mont_Bal'] = (clean_num(df_bal[st.session_state.immos_col_cred_f1])
                                        - clean_num(df_bal[st.session_state.immos_col_debit_f1]))
                else:
                    df_b['Mont_Bal'] = clean_num(df_bal[st.session_state.immos_col_mont_f1])
                return (df_b[['PCEC','Mont_Bal']].query('PCEC.str.len()>0')
                        .groupby('PCEC', as_index=False)['Mont_Bal'].sum())

            # ── Build table générique avec son propre mapping ─────────────────
            def build_table(map_pci_col, map_pcec_col, pci_col, mont_col, label_mont, diag_key):
                df_b = get_balance_df()
                if df_b is None:
                    st.error("❌ Balance non configurée.")
                    return pd.DataFrame()

                # Mapping propre à ce tableau
                df_m = df_map[[map_pci_col, map_pcec_col]].copy()
                df_m.columns = ['PCI','PCEC']
                df_m['PCI']  = clean_code(df_m['PCI'])
                df_m['PCEC'] = clean_code(df_m['PCEC'])
                df_m = (df_m.dropna(subset=['PCI','PCEC'])
                        .query('PCI.str.len()>0 and PCEC.str.len()>0'))
                ...
                df_i = (df_i.dropna(subset=['PCI']).query('PCI.str.len()>0')
                        .groupby('PCI', as_index=False)['Mont_Immos'].sum())

                # Diagnostic
                pci_f3  = set(df_i['PCI'].unique())
                pci_map = set(df_m['PCI'].unique())
                communs = pci_f3 & pci_map
                absents = pci_f3 - pci_map
                with st.expander(f"🔍 Diagnostic {diag_key}", expanded=(len(communs)==0)):
                    c1,c2,c3 = st.columns(3)
                    c1.metric("PCI fichier 3", len(pci_f3))
                    c2.metric("PCI mapping",   len(pci_map))
                    c3.metric("En commun",     len(communs),
                              delta="✅ OK" if len(communs)>0 else "❌ 0",
                              delta_color="normal" if len(communs)>0 else "inverse")
                    if len(communs) == 0:
                        st.error("❌ Aucun PCI en commun — vérifiez les colonnes mapping sélectionnées.")
                        st.write("**PCI fichier 3 (exemples) :**", sorted(list(pci_f3))[:8])
                        st.write("**PCI mapping (exemples) :**",   sorted(list(pci_map))[:8])
                    elif len(absents) > 0:
                        st.warning(f"⚠️ {len(absents)} PCI absents du mapping")
                        st.write("**Non mappés :**", sorted(list(absents))[:10])

                # Jointures
                df_r = pd.merge(df_i, df_m, on='PCI', how='left')
                df_r['PCEC']     = df_r['PCEC'].fillna('— NON MAPPÉ —')
                ...
                df_r.columns = ['PCI','PCEC', label_mont, 'Montant Balance (Compta)', 'Écart']
                return df_r

            def show_table(df_res, key_prefix):
                if df_res is None or df_res.empty:
                    st.warning("⚠️ Aucun résultat.")
                    return
                mc = df_res.columns[2]; bc = df_res.columns[3]; ec = df_res.columns[4]
                k1,k2,k3,k4 = st.columns(4)
                k1.metric("Lignes",          len(df_res))
                k2.metric(mc[:18],           f"{df_res[mc].sum():,.0f}".replace(',',' '))
                k3.metric("Total Balance",   f"{df_res[bc].sum():,.0f}".replace(',',' '))
                k4.metric("Écart total",     f"{df_res[ec].sum():,.0f}".replace(',',' '),
                          delta=f"{df_res[ec].sum():,.0f}".replace(',',' '),
                          delta_color="inverse" if df_res[ec].sum()!=0 else "off")
                f1,f2 = st.columns(2)
                srch = f1.text_input("🔍 PCI / PCEC", key=f"{key_prefix}_srch")
                only_ec = f2.checkbox("Écarts ≠ 0", key=f"{key_prefix}_ec")
                df_d = df_res.copy()
                if srch:
                    df_d = df_d[df_d['PCI'].str.contains(srch,case=False,na=False)|
                                df_d['PCEC'].str.contains(srch,case=False,na=False)]
                if only_ec:
                    df_d = df_d[df_d[ec]!=0]
                df_fmt = df_d.copy()
                for c in [mc,bc,ec]:
                ...
                with ce2:
                    ...
                    with pd.ExcelWriter(out, engine='openpyxl') as w:
                        df_res.to_excel(w, index=False, sheet_name=key_prefix[:31])
                    st.download_button("📥 Télécharger Excel", data=out.getvalue(),
                        file_name=f"{key_prefix}_{datetime.now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True, key=f"{key_prefix}_dl")

            # ══════════════════════════════════════════════════════════════════
            # CONFIG BALANCE (commune aux 3 tableaux)
            # ══════════════════════════════════════════════════════════════════
            bal_ok = st.session_state.immos_col_pcec_f1 is not None
            with st.expander("⚙️ Config Balance PCEC (commune)", expanded=not bal_ok):
                st.markdown("**Fichier 1 — Balance**")
                c1a,c1b = st.columns(2)
                with c1a:
                    _pcec_f1 = st.selectbox("Colonne PCEC", cols_f1,
                ...
                with c1b:
                    ...
                    if is_txt:
                        st.info("✅ TXT — montant auto")
                        _mode="direct"; _mont_f1='Montant'; _cred_f1=_deb_f1=None
                    else:
                        _mode = st.radio("Mode", ["Créditeur − Débiteur","Colonne directe"],
                            index=0 if st.session_state.immos_mode_mont!="direct" else 1,
                            key="cfg_mode_mont", horizontal=True)
                        if _mode=="Créditeur − Débiteur":
                            cc1,cc2=st.columns(2)
                            _cred_f1=cc1.selectbox("Solde Créditeur",cols_f1,
                                index=safe_idx(cols_f1,st.session_state.immos_col_cred_f1,
                                               safe_idx(cols_f1,auto(cols_f1,'CREDIT','CRÉDIT'))),
                                key="cfg_cred_f1")
                            _deb_f1=cc2.selectbox("Solde Débiteur",cols_f1,
                                index=safe_idx(cols_f1,st.session_state.immos_col_debit_f1,
                                               safe_idx(cols_f1,auto(cols_f1,'DEBIT','DÉBIT'))),
                                key="cfg_debit_f1")
                            _mont_f1=None
                        else:
                            _mont_f1=st.selectbox("Colonne Montant",cols_f1,
                                index=safe_idx(cols_f1,st.session_state.immos_col_mont_f1,
                                               safe_idx(cols_f1,auto(cols_f1,'MONTANT','SOLDE'))),
                                key="cfg_mont_f1")
                            _cred_f1=_deb_f1=None
                _,sv2,_=st.columns([1,2,1])
                with sv2:
                    if st.button("💾 Sauvegarder Balance", type="primary",
                                  use_container_width=True, key="btn_save_bal"):
                        st.session_state.immos_col_pcec_f1  = _pcec_f1
                        st.session_state.immos_mode_mont    = _mode
                        st.session_state.immos_col_mont_f1  = _mont_f1
                        st.session_state.immos_col_cred_f1  = _cred_f1
                        st.session_state.immos_col_debit_f1 = _deb_f1
            ...
            if not bal_ok:
                st.info("💡 Sauvegardez d'abord la config Balance ci-dessus.")
            else:
                # ── helper : bloc config d'un tableau ─────────────────────────
                def render_tableau(num, icon, default_label,
                                   key_label, key_map_pci, key_map_pcec,
                                   key_pci, key_mont, key_res,
                                   key_save, key_calc,
                                   auto_map_pci, auto_map_pcec,
                                   auto_pci, auto_mont):

                    lbl = st.session_state[key_label] or default_label
                    st.markdown(f"### {icon} Tableau {num} — {lbl}")
                    cfg_ok = st.session_state[key_map_pci] is not None

                    with st.expander(f"⚙️ Configuration Tableau {num}", expanded=not cfg_ok):

                        # ── Nom du tableau ────────────────────────────────────
                        _lbl = st.text_input("Nom du tableau",
                            value=st.session_state[key_label] or default_label,
                            key=f"cfg_{key_label}")

                        st.markdown("**Mapping (Fichier 2)**")
                        ma, mb = st.columns(2)
                        _map_pci  = ma.selectbox("Colonne PCI  dans le mapping", cols_f2,
                            index=safe_idx(cols_f2, st.session_state[key_map_pci],
                                           safe_idx(cols_f2, auto(cols_f2, *auto_map_pci))),
                            key=f"cfg_{key_map_pci}")
                        _map_pcec = mb.selectbox("Colonne PCEC dans le mapping", cols_f2,
                            index=safe_idx(cols_f2, st.session_state[key_map_pcec],
                                           safe_idx(cols_f2, auto(cols_f2, *auto_map_pcec))),
                            key=f"cfg_{key_map_pcec}")

                        st.markdown("**Fichier 3**")
                        fa, fb = st.columns(2)
                        _pci  = fa.selectbox("Colonne PCI  (fichier 3)", cols_f3,
                            index=safe_idx(cols_f3, st.session_state[key_pci],
                                           safe_idx(cols_f3, auto(cols_f3, *auto_pci))),
                            key=f"cfg_{key_pci}")
                        _mont = fb.selectbox("Colonne Montant (fichier 3)", cols_f3,
                            index=safe_idx(cols_f3, st.session_state[key_mont],
                                           safe_idx(cols_f3, auto(cols_f3, *auto_mont))),
                            key=f"cfg_{key_mont}")

                        _,sb2,_=st.columns([1,2,1])
                        with sb2:
                            if st.button(f"💾 Sauvegarder Tableau {num}",
                                          use_container_width=True, key=key_save):
                                st.session_state[key_label]    = _lbl
                                st.session_state[key_map_pci]  = _map_pci
                                st.session_state[key_map_pcec] = _map_pcec
                                st.session_state[key_pci]      = _pci
                                st.session_state[key_mont]     = _mont
                                st.session_state[key_res]      = None
                                st.success(f"✅ Config Tableau {num} sauvegardée")
                                st.rerun()

                    if cfg_ok:
                        _,cb2,_=st.columns([1,2,1])
                        with cb2:
                            if st.button(f"🔗 Calculer Tableau {num} — {lbl}",
                                          type="primary", use_container_width=True, key=key_calc):
                                with st.spinner(f"Calcul T{num}..."):
                                    st.session_state[key_res] = build_table(
                                        st.session_state[key_map_pci],
                                        st.session_state[key_map_pcec],
                                        st.session_state[key_pci],
                                        st.session_state[key_mont],
                                        f'Montant {st.session_state[key_mont]}',
                                        diag_key=f"T{num}")
                        if st.session_state[key_res] is not None:
                            show_table(st.session_state[key_res], f"T{num}_{lbl}")

                st.divider()
                render_tableau(
                    num=1, icon="🏗️", default_label="Bilan Immos",
                    key_label='t1_label',
                    key_map_pci='t1_map_pci', key_map_pcec='t1_map_pcec',
                    key_pci='t1_pci_col',     key_mont='t1_mont_col',
                    key_res='immos_res_t1',
                    key_save='btn_save_t1',   key_calc='btn_calc_t1',
                    auto_map_pci =('PCI BILAN','PCI_BILAN','PCI'),
                    auto_map_pcec=('PCEC BILAN','PCEC_BILAN','PCEC'),
                    auto_pci     =('IMMO_COMPTE','CPTE_IMMO','IMMO'),
                    auto_mont    =('COUT','COÛT','COST'),
                )

                st.divider()
                render_tableau(
                    num=2, icon="📉", default_label="Résultat Dotations",
                    key_label='t2_label',
                    key_map_pci='t2_map_pci', key_map_pcec='t2_map_pcec',
                    key_pci='t2_pci_col',     key_mont='t2_mont_col',
                    key_res='immos_res_t2',
                    key_save='btn_save_t2',   key_calc='btn_calc_t2',
                    auto_map_pci =('PCI RESULTAT','PCI_RESULTAT','PCI'),
                    auto_map_pcec=('PCEC RESULTAT','PCEC_RESULTAT','PCEC'),
                    auto_pci     =('DOTATION_COMPTE','CPTE_DOT','DOTATION'),
                    auto_mont    =('DOTATION_EX','EXERCICE','DOT'),
                )

                st.divider()
                render_tableau(
                    num=3, icon="📋", default_label="Tableau 3",
                    key_label='t3_label',
                    key_map_pci='t3_map_pci', key_map_pcec='t3_map_pcec',
                    key_pci='t3_pci_col',     key_mont='t3_mont_col',
                    key_res='immos_res_t3',
                    key_save='btn_save_t3',   key_calc='btn_calc_t3',
                    auto_map_pci =('PCI RESULTAT','PCI_RESULTAT','PCI'),
                    auto_map_pcec=('PCEC RESULTAT','CUMUL','PCEC'),
                    auto_pci     =('PCI_RESULTAT','CPTE_RES','RESULTAT'),
                    auto_mont    =('CUMUL','AMORT','VALEUR'),
                )
        ...
        with tab2:
            ...
            with ref_col2:
                ...
                <div style="background:#f8f9fa;border-left:5px solid {color_mla};
                    ...
                    <br><small style="color:#6c757d;">Colonnes : Code_Agence · Code_Localite · Localite · DRPP</small>
                </div>
                """, unsafe_allow_html=True)
            ...
            if not all_ok_tab:
                ...
                # ── Sélecteur unité BAM ──────────────────────────────────────
                st.markdown("##### ⚙️ Unité des montants BAM")
                bam_unit_options = {
                    "Milliers de DH (× 1 000)"    : 1_000,
                    "Millions de DH (× 1 000 000)" : 1_000_000,
                    "Dirhams bruts (× 1)"          : 1,
                    "Milliards de DH (× 1 000 000 000)" : 1_000_000_000,
                }
                bam_unit_label = st.selectbox(
                    "Les montants dans votre fichier BAM sont exprimés en :",
                    options=list(bam_unit_options.keys()),
                    index=0,
                    key="bam_unit_sel",
                    help="Cette valeur sert à convertir les montants BAM en DH pour calculer les PDM avec Saham (qui est en DH bruts)."
                )
                bam_unit_factor = bam_unit_options[bam_unit_label]
                st.session_state['bam_unit_factor'] = bam_unit_factor
                ...
                if mode_s_tab != "Tout" and ann_s_tab and '_ac' in df_saham_filtered_tab.columns:
                    df_saham_filtered_tab = df_saham_filtered_tab[
                        df_saham_filtered_tab['_ac'] == str(ann_s_tab)]
                if mode_s_tab == "Année + Mois" and mois_s_tab and '_mc2' in df_saham_filtered_tab.columns:
                    # Normaliser la valeur du mois pour comparer sans ambiguité
                    mois_str = str(int(float(str(mois_s_tab))))
                    df_saham_filtered_tab = df_saham_filtered_tab[
                        df_saham_filtered_tab['_mc2'].astype(str)
                        .str.strip().str.replace(r'\.0$','',regex=True) == mois_str]
                elif mode_s_tab == "Année entière" and '_mc2' in df_saham_filtered_tab.columns:
                    mois_dispo_tab = []
                    for m in df_saham_filtered_tab['_mc2'].dropna().unique():
                        m_clean = str(m).strip().replace('.0','')
                        if m_clean.isdigit() and 1 <= int(m_clean) <= 12:
                            mois_dispo_tab.append(int(m_clean))
                    if mois_dispo_tab:
                        max_mois_str = str(max(mois_dispo_tab))
                        df_saham_filtered_tab = df_saham_filtered_tab[
                            df_saham_filtered_tab['_mc2'].astype(str)
                            .str.strip().str.replace(r'\.0$','',regex=True) == max_mois_str]
                    ...
                    if st.button("🚀 Calculer les PDM par Localité", type="primary",
                        ...
                        with st.spinner("⏳ Traitement en cours..."):
                            try:
                                ...
                                df_pdm_tab, df_pdm_tab_detail = process_saham_pdm(
                                ...
                                # Sauvegarder tableau_f_detail directement depuis le tuple retourné
                                st.session_state.saham_tableau_f_detail = df_pdm_tab_detail
                                if df_pdm_tab_detail is not None:
                                    save_cache("saham_tableau_f_detail", df_pdm_tab_detail)
                                else:
                                    clear_cache("saham_tableau_f_detail")
